...
export function DashboardClient() {
  ...
  const checkConnection = async () => {
    try {
      ...
      setError(connected ? null : 'UPSTOX_ANALYTICS_TOKEN is not configured on the server.');
    } catch {
      setIsConnected(false);
      setError('Unable to check Upstox configuration.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        setError(data.message || 'Upstox Analytics Token is not configured.');
      }
    } catch {
      setError('Failed to verify Upstox configuration.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleScan = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/upstox/prime-scan', { cache: 'no-store' });
      const data: ScannerResponse = await res.json();
      if (!res.ok || data.status !== 'success') throw new Error(data.message || 'Failed to run Prime scan');
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="mx-auto max-w-[1800px] px-4 py-6">
        ...
        {scanData && <div className="space-y-6"><SummaryCards summary={scanData.summary} /><div><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold text-white">Prime Scanner Results</h2><div className="text-sm text-slate-400">{scanData.results.length} stocks scanned{scanData.summary.failedCount ? ` • ${scanData.summary.failedCount} unavailable` : ''}</div></div><ScannerTable results={scanData.results} onSelectStock={setSelectedStock} selectedStock={selectedStock} /></div><div className="rounded-lg border border-amber-600/30 bg-amber-500/10 p-4 text-sm text-amber-400"><p className="font-semibold">IMPORTANT DISCLAIMER:</p><p className="mt-2">This scanner is for educational and research purposes only. Confirmation rules are NOT VERIFIED. Do not use for actual trading without proper validation. No trading signals should be considered investment advice.</p></div></div>}
      </main>
