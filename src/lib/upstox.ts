...
export class UpstoxService {
  ...
  async getHistoricalCandles(instrumentKey: string, interval: '1minute' | '5minute' | '30minute' | 'day' | 'week' | 'month', toDate: string, fromDate?: string): Promise<Candle[]> {
    ...
    const path = minuteInterval ? (fromDate ? `/historical-candle/${encodeURIComponent(instrumentKey)}/minutes/${minuteInterval}/${toDate}/${fromDate}` : `/historical-candle/${encodeURIComponent(instrumentKey)}/minutes/${minuteInterval}/${toDate}`) : (fromDate ? `/historical-candle/${encodeURIComponent(instrumentKey)}/${interval}/${toDate}/${fromDate}` : `/historical-candle/${encodeURIComponent(instrumentKey)}/${interval}/${toDate}`);
    try {
      // A slow/blocked instrument must not hold the whole 210-stock scan hostage.
      const response = await axios.get<UpstoxHistoricalCandleResponse>(`${UPSTOX_API_V3_BASE}${path}`, { headers: this.authHeaders(), timeout: 3500 });
  ...
  async getIntradayCandles(instrumentKey: string, interval: '1' | '5' | '30' = '5'): Promise<Candle[]> {
    try {
      const response = await axios.get<UpstoxHistoricalCandleResponse>(`${UPSTOX_API_V3_BASE}/historical-candle/intraday/${encodeURIComponent(instrumentKey)}/minutes/${interval}`, { headers: this.authHeaders(), timeout: 3500 });
      return this.mapCandleResponse(response);
    } catch (error) {
      const status = axios.isAxiosError(error) ? error.response?.status : undefined;
      const apiMessage = axios.isAxiosError(error) ? error.response?.data?.errors?.[0]?.message || error.response?.data?.message : undefined;
  ...
  async getAllNSEFuturesInstruments(): Promise<Record<string, UpstoxFuturesInstrument>> {
    const rows = await getNSEInstrumentRows(); const now = Date.now(); const result: Record<string, UpstoxFuturesInstrument> = {};
