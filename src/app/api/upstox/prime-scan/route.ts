...
/** Start <=50 historical requests/sec, but keep 100 workers available so slow API calls
 * do not leave the rate-limit slots idle. Upstox standard APIs allow 50 req/sec. */
...
export async function GET() {
  try {
    if (!upstoxService.isAuthenticated()) return NextResponse.json({ status: 'error', error: 'Upstox Analytics Token is not configured', message: 'Set UPSTOX_ANALYTICS_TOKEN on the server.' }, { status: 401 });
    ...
    if (!instruments.length) return NextResponse.json({ status: 'error', error: 'No NSE equity instruments matched the active F&O stock universe.', message: 'Upstox returned no underlying NSE cash instruments for the F&O universe.' }, { status: 503 });
    ...
    const scanOne = async (instrument: (typeof instruments)[number]) => {
      ...
      try {
        ...
        if (candles.length < 30) { failedCount += 1; return; }

        const sessionDates = [...new Set(candles.map(c => istDate(c.timestamp)))].sort();
        if (sessionDates.length < 2) { failedCount += 1; return; }
