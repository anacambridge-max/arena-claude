...
async function readApiResponse<T>(res: Response): Promise<T> {
  ...
  try {
    ...
    throw new Error(`Server returned a non-JSON response (HTTP ${res.status}). ${text.replace(/\s+/g, ' ').slice(0, 180)}`);
  }
}
...
export function DashboardClient() {
  ...
  const checkConnection = async () => {
    try {
      ...
      setError(stringifyError(err) || 'Unable to check Upstox configuration.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (res.ok && data.status === 'success') {
        ...
        setError(stringifyError(data.message) || `Upstox verification failed (HTTP ${res.status}).`);
      }
    } catch (err) {
      setError(stringifyError(err) || 'Failed to verify Upstox configuration.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleScan = async () => {
    ...
    setError(null);
    try {
      // Do not abort the browser request on a timer. A full 210-stock scan can
      // legitimately take longer than a short client-side timeout, while the
      // Vercel function has its own server-side maxDuration.
      ...
      if (!res.ok || data.status !== 'success' || !data.data) {
        throw new Error(stringifyError(data.message) || stringifyError(data.error) || `Failed to run Prime scan (HTTP ${res.status}).`);
      }
      setScanData(data.data);
      setLastUpdate(new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }));
    } catch (err) {
      if (requestId === scanRequestIdRef.current) {
        setError(stringifyError(err) || 'Failed to run Prime scan');
      }
    } finally {
      if (requestId === scanRequestIdRef.current) setIsLoading(false);
    }
  };
